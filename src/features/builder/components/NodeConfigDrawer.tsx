      ...
      {data.kind === "prompt" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <textarea
            ...
            onChange={(event) => updateNodeData(selectedNode.id, { prompt: event.target.value })}
            style={{ minHeight: 80, padding: 8, borderRadius: 8, border: "1px solid #cbd5f5" }}
          />
        </div>
      )}
      {data.kind === "ai_text" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <select
            ...
            onChange={(event) => updateNodeData(selectedNode.id, { provider: event.target.value as AITextNodeData["provider"] })}
          ...
          <input
            ...
            style={{ padding: 8, borderRadius: 8, border: "1px solid #cbd5f5" }}
          />
          <label style={{ fontSize: 12, color: "#475569" }}>Temperature</label>
          <input
            type="number"
            min={0}
            max={1}
            step={0.1}
            value={(data as AITextNodeData).temperature}
            onChange={(event) => updateNodeData(selectedNode.id, { temperature: Number(event.target.value) })}
            style={{ padding: 8, borderRadius: 8, border: "1px solid #cbd5f5" }}
          />
        </div>
      )}
      {data.kind === "ai_image" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <select
            ...
            onChange={(event) => updateNodeData(selectedNode.id, { provider: event.target.value as AIImageNodeData["provider"] })}
            style={{ padding: 8, borderRadius: 8, border: "1px solid #cbd5f5" }}
          >
            <option value="openai">openai</option>
            <option value="gemini">gemini</option>
          ...
          <input
            ...
            onChange={(event) => updateNodeData(selectedNode.id, { size: event.target.value })}
            style={{ padding: 8, borderRadius: 8, border: "1px solid #cbd5f5" }}
          />
        </div>
      )}
